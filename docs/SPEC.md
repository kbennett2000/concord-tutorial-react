...
| **T9** | CI: three-engine smoke + Lesson 5 build gate | `tools/screenshots/smoke.mjs` driving every lesson in Chromium/Firefox/WebKit (**waits for React/Babel to mount before asserting** — the #root is empty until Babel compiles), workflow steps for `npm ci && npm run build` on `compare-app` (the build gate — also produces the `dist/` the smoke drives via `vite preview`), Playwright install, and the smoke; the committed `compare-app/package-lock.json` (so `npm ci` works); `"smoke"` script. **Proof: full CI green against a live v1.1.0 Concord.** | T5, T6, T8, Concord v1.1.0 |
